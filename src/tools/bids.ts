...
export const setKeywordBidsSchema = z.object({
  keyword_ids: z.array(z.number()).optional().describe("ID ключевых фраз (ставки на уровне фраз)"),
  ad_group_ids: z.array(z.number()).optional().describe("ID групп (ставки на все фразы групп)"),
  campaign_ids: z.array(z.number()).optional().describe("ID кампаний (ставки на все фразы кампаний)"),
...
export async function handleSetKeywordBids(params: z.infer<typeof setKeywordBidsSchema>): Promise<string> {
  ...
  const Bids = ids.map((id) => ({ [field]: id, ...base }));
